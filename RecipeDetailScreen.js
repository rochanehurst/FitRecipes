...
export default function RecipeDetailScreen({ route, navigation }) {
  // Expect a full recipe object in params
  const recipe = route?.params?.recipe;

  // Friendly guard if someone navigates here without data
  ...
  const NutritionalRow = () => (
    <View style={styles.nutritionalSection}>
      ...
      <Text style={styles.dietaryInfo}>
        Vegan: {recipe?.vegan ? 'Yes' : 'No'} | Gluten Free:{' '}
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#FFFFFF' }}>
      ...
      <View style={[styles.header, { marginTop: topBarHeight }]}>
        ...
        {/* Home (pop-to-top if possible; otherwise switch tab) */}
        <TouchableOpacity
          onPress={() => {
            if (navigation.canGoBack()) {
              // Matches the back-style slide animation
              navigation.popToTop();
              return;
            }
      ...
      <View style={styles.imageContainer}>
        <Image
          source={{
            uri: recipe.image || 'https://via.placeholder.com/800x400?text=No+Image',
          }}
